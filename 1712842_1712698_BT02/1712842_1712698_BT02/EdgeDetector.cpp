...
int EdgeDetector::DetectBySobel(const Mat& sourceImage, Mat& destinationImage, int kWidth, int kHeight)
{
	...
	Mat GxImage, GyImage;
	 GxImage.create(grayImg.size(), grayImg.type());
	 GyImage.create(grayImg.size(), grayImg.type());
	...
	{// Tính toán theo mặt nạ Sobel để có được biên cạnh
		for (int k = 1; k < grayImg.cols - 1; k++)
		{
			...
			GxImage.at<uchar>(i,k) = static_cast<uchar>(Gx);
			GyImage.at<uchar>(i, k) = static_cast<uchar>(Gy);
...
int EdgeDetector::DetectByCanny(const Mat& sourceImage, Mat& destinationImage)
{
	...
	float Gp, Ga, Gb;
	Gp = Ga = Gb = -INFINITY;
	uchar* pAngle = (uchar*)angleImage.data;
	uchar *pAngleRow;
